...
public class VernacularClient {
    ...
    /**
     * Starts the VNC client using the specified input and output streams.
     * This bypasses Socket creation, allowing the VNC protocol to run over
     * any transport (e.g. a network tunnel).
     *
     * @param in  InputStream for reading from the VNC server
     * @param out OutputStream for writing to the VNC server
     * @throws IllegalStateException if the client is already running
     */
    public void start(InputStream in, OutputStream out) {
        ...
        try {
            session = new VncSession(config, new BufferedInputStream(in), out);
